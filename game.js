...
function move() {
  ...
  if (downPressed) {
    ...
    const element = document.elementFromPoint(player.offsetLeft, newTop + 50);
  ...
  if (upPressed) {
    ...
    const element = document.elementFromPoint(player.offsetLeft, newTop);
    if (element.classList.contains('sky') === false) {
      player.style.top = `${newTop}px`;
    }
  ...
  if (leftPressed) {
    ...
    const element = document.elementFromPoint(newLeft, player.offsetTop);
    if (element.classList.contains('sky') === false) {
  ...
  if (rightPressed) {
    ...
    const element = document.elementFromPoint(newLeft + 32, player.offsetTop);
    if (element.classList.contains('sky') === false) {
      player.style.left = `${newLeft}px`;
    }
...
function clearEnemy() {
  ...
  const enemies = document.getElementsByClassName('alien');
